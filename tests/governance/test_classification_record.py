...
class ClassificationRecordTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.frozen = load_frozen_inputs(PROJECT_ROOT)
    ...
    def test_frozen_input_tamper_fails_closed(self) -> None:
        original = PROJECT_ROOT / "configs/governance/ANALYZER_LIMITS_V1.json"
        data = original.read_bytes()
        try:
            original.write_bytes(data + b" ")
            with self.assertRaises(FrozenInputError):
                load_frozen_inputs(PROJECT_ROOT)
        finally:
