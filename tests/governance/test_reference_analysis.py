...
class ReferenceAnalysisTests(unittest.TestCase):
    ...
    def test_unknown_tracked_types_default_to_unsupported(self) -> None:
        self._write("notes/design.md", "consumer reference\n")
        self._write("stubs/api.pyi", "VALUE: int\n")
        self._write("images/chart.png", "not parsed as semantics\n")
        self._write("upper/module.PY", "VALUE = 1\n")
        ...
        result = analyze_reference_snapshot(
            str(self.repo),
            commit_sha1=head,
            changed_nodes=(b"notes/design.md",),
            manifest=_manifest(protected_modules=()),
            limits=LIMITS,
        )
    ...
    def test_unknown_type_cardinality_limit_fails_closed(self) -> None:
        self._write("consumer.one", "one\n")
        self._write("consumer.two", "two\n")
        head = self._commit()
        limits = dict(LIMITS)
        limits["max_unsupported_reference_file_types"] = 1

        with self.assertRaisesRegex(
            ReferenceAnalysisError,
            "max_unsupported_reference_file_types=1",
        ):
            analyze_reference_snapshot(
                str(self.repo),
                commit_sha1=head,
                changed_nodes=(b"consumer.one",),
                manifest=_manifest(protected_modules=()),
                limits=limits,
            )
