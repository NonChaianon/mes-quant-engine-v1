...
class ReferenceParserTests(unittest.TestCase):
    ...
    def test_namespace_subscript_evasion_is_unresolved(self) -> None:
        cases = {
            "builtins-subscript": b"__builtins__['eval']('1')\n",
            "builtins-alias": (
                b"b = __builtins__\n"
                b"b['eval']('1')\n"
            ),
            "module-dict": (
                b"import builtins\n"
                b"builtins.__dict__['eval']('1')\n"
            ),
            "importlib-dict": (
                b"import importlib\n"
                b"importlib.__dict__['import_module']('a')\n"
            ),
            "sys-modules": (
                b"import sys\n"
                b"sys.modules['builtins'].eval('1')\n"
            ),
            "namespace-callable-alias": (
                b"import builtins\n"
                b"d = builtins.__dict__\n"
                b"f = d['eval']\n"
                b"f('1')\n"
            ),
        }
        ...
        for case, source in cases.items():
            with self.subTest(case=case):
                parsed = _parse(b"module.py", source)
                unresolved = {
                    (item.reference_kind, item.reason_code)
                    for item in parsed.unresolved
                }
