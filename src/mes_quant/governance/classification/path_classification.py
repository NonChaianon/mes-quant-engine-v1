...
class PathClassificationError(RuntimeError):
    """Raised when trusted path/manifest classification cannot complete safely."""
...
def _validate_semantic_manifest_entries(
    manifest: dict[str, Any],
) -> None:
    """Validate identities; Phase 2 resolves them from immutable Git objects."""

    modules = manifest.get(
        "protected_quant_modules"
    )
    symbols = manifest.get(
        "protected_symbols"
    )

    if not isinstance(modules, list):
        raise PathClassificationError(
            "protected_quant_modules must be a list"
        )

    if not isinstance(symbols, list):
        raise PathClassificationError(
            "protected_symbols must be a list"
        )
    ...
    for module in modules:
        if (
            ...
            raise PathClassificationError(
                "invalid protected_quant_modules entry: "
                f"{module!r}"
            )
    ...
    for symbol in symbols:
        if (
            ...
            raise PathClassificationError(
                "invalid protected_symbols entry: "
                f"{symbol!r}"
            )
...
def classify_paths(
    ...
    """Classify raw Git paths using frozen path categories.

    Semantic module/symbol identities are validated here and resolved by
    Phase 2 immutable-tree reference analysis. An otherwise unclassified
    path becomes CROSS_BOUNDARY. UX_ONLY remains unreachable while the
    frozen presentation boundary is empty.
    """

    _validate_semantic_manifest_entries(manifest)
