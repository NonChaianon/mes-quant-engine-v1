...
def _iter_nul_fields(stream: object) -> object:
    """Yield NUL-delimited raw Git fields without buffering all stdout."""
    ...
    if buffer:
        raise GitDeltaError("unterminated raw Git diff field")


def _terminate_process(process: subprocess.Popen[bytes]) -> None:
    """Stop a Git producer immediately after fail-close detection."""
...
def canonical_git_tree_delta(
    ...
    *,
    ...
    max_tree_delta_entries: int,
) -> tuple[DeltaEntry, ...]:
    """Derive a bounded, rename-disabled, raw-path-byte canonical tree delta."""

    if (
        not isinstance(max_tree_delta_entries, int)
        or isinstance(max_tree_delta_entries, bool)
        or max_tree_delta_entries < 1
    ):
        raise GitDeltaError("max_tree_delta_entries must be a positive integer")
    ...
    try:
        ...
        raise GitDeltaError(f"cannot start git diff-tree: {exc}") from exc

    if process.stdout is None or process.stderr is None:
        _terminate_process(process)
        raise GitDeltaError("git diff-tree pipes unavailable")

    entries: list[DeltaEntry] = []

    try:
        fields = iter(_iter_nul_fields(process.stdout))
        ...
        while True:
            ...
            if len(parts) != 5:
                raise GitDeltaError(
                    f"unexpected raw diff header field count: {header!r}"
                )
            ...
            if status[:1] in {b"R", b"C"}:
                raise GitDeltaError(
                    "rename/copy status is forbidden by --no-renames authority"
                )

            try:
                path = next(fields)
            except StopIteration as exc:
                raise GitDeltaError("raw diff path missing") from exc

            if not path:
                raise GitDeltaError("empty Git path is invalid")

            if len(entries) >= max_tree_delta_entries:
                raise GitDeltaError(
                    "ANALYZER_RESOURCE_LIMIT_EXCEEDED: "
                    f"max_tree_delta_entries={max_tree_delta_entries}"
                )
    ...
    entries.sort(
        key=lambda item: (
            item.new_path_bytes
            if item.new_path_bytes is not None
            else item.old_path_bytes or b"",
