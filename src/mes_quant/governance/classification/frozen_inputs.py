...
EXPECTED_SHA256 = {
    ...
    _SPEC_PATH: "C893BBC4132F222766A7427293F45E5CF617383CF97C4398586DAA50DB2DE18D",
}
...
def load_frozen_inputs(
    ...
    *,
    ...
    authority = _resolve_authority_commit(
        repo,
        authority_commit_sha1,
    )

    spec_data = _read_authority_blob(
        repo,
        authority,
        _SPEC_PATH,
    )
    ...
    if (
        spec_data.startswith(b"\xef\xbb\xbf")
        or b"\r" in spec_data
        or not spec_data.endswith(b"\n")
    ):
        raise FrozenInputError(
            f"byte policy failure: {_SPEC_PATH}"
        )

    classifier_spec_sha256 = (
        hashlib.sha256(spec_data)
        .hexdigest()
        .upper()
    )

    expected_spec_sha256 = EXPECTED_SHA256[
        _SPEC_PATH
    ]

    if classifier_spec_sha256 != expected_spec_sha256:
        raise FrozenInputError(
            "frozen input SHA-256 mismatch for "
            f"{_SPEC_PATH}: "
            f"{classifier_spec_sha256} != "
            f"{expected_spec_sha256}"
        )

    manifest, manifest_sha = _load_exact_json(
        repo,
        authority,
        _MANIFEST_PATH,
    )
