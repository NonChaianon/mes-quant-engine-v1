...
def _unsupported_reference_type(path: bytes) -> str | None:
    """Classify every readable tracked blob lacking a governed V1 parser."""
    ...
    if b"." in basename:
        suffix = b"." + basename.rsplit(b".", 1)[-1].lower()
        return "extension/" + suffix.decode(
            "ascii",
            "backslashreplace",
        )
...
def _add_graph_targets(
    adjacency: dict[bytes, set[bytes]],
    source: bytes,
    targets: Iterable[bytes],
    *,
    graph_edge_count: int,
    max_graph_edges: int,
) -> int:
    """Add unique graph edges while enforcing the frozen edge ceiling."""

    source_targets = adjacency[source]
    ...
    *,
    ...
    for item in tracked_objects:
        ...
        if item.mode == "120000":
            ...
            graph_edge_count = _add_graph_targets(
                adjacency,
                path,
                targets,
    ...
    recomputed_graph_edge_count = 0

    for targets in canonical_adjacency.values():
        graph_nodes.update(targets)
        recomputed_graph_edge_count += len(targets)

    if recomputed_graph_edge_count != graph_edge_count:
        raise ReferenceAnalysisError(
            "graph edge accounting mismatch"
        )
